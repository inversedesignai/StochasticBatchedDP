...
This project introduces **stochastic segmented dynamic programming** (SSDP), a scaling
framework that pushes the tractable joint-DP horizon to arbitrary `K_total` while
preserving exactness of inner Bellman backups within each segment and unbiasedness of
per-sample outer-hardware gradients.

The framework combines four ingredients:

1. **Segmented DP**: decompose `K_total = n × K_segment` into `n` consecutive segments of length
   `K_segment`, each a tractable exact Bellman solve from its predecessor's terminal belief.

2. **Stochastic outer optimization**: at each outer-gradient step, sample `M` trajectories
   under the current segmented policy; bypass the exhaustive enumeration over reachable
   segment-boundary beliefs.

3. **Per-sample sharp-max envelope-theorem gradients**: each sampled segment's local Bellman
   gradient is exact, no smoothing temperature.

4. **Per-segment advantage variance reduction**: the score-function term in the REINFORCE
   identity is paired per segment with a remaining-horizon-value baseline, reducing variance
   while preserving unbiasedness.

The resulting outer-hardware gradient is unbiased for the segmented-policy value and has
...
variable) × (exact Bellman dynamic programming as inner solver in each segment) ×
(stochastic Monte-Carlo sampling of segment-boundary trajectories) × (sharp-max
envelope-theorem gradients) × (per-segment advantage baselines) is not present in the
...
- `StochasticSegmentedDP.tex` — formal article: setup, full gradient derivation including
  detailed n>2 case, algorithm, convergence, connections to existing methods
- `StochasticSegmentedDP.pdf` — compiled article (15 pages)
...
existing exact-Bellman infrastructure provides a clean baseline at `K_total = K_segment =
4`; (b) the Ramsey likelihood and count-tuple sufficient statistic match the SSDP
framework cleanly; (c) the wide-prior multimodal regime (`φ_max → Φ_0/2`) is exactly the
regime where exact DP fails and SSDP's long-horizon disambiguation should shine.
...
- Demonstrate SSDP at `K_total ∈ {8, 16, 32}` with `K_segment = 4` on the scqubit problem.
...
- Headline: SSDP achieves `~N×` MSE reduction over the best classical baseline at
  `K_total = N`.
...
### Tier 3: SSDP as a general-purpose POMDP scaling primitive
...
- Headline: SSDP as a unifying scaling framework for joint hardware-policy co-design across
...
```
Stochastic Segmented DP for joint outer-c, inner-π optimization

Input:
  c_0 : initial outer parameter
  K_total : total horizon
  K_segment : per-segment horizon (chosen so exact Bellman is tractable)
...
for t = 1 ... T_outer:
    g ← 0
    for m = 1 ... M (parallel):
        # Sample one trajectory of length K_total under the segmented policy at current c
        b ← prior
        record actions, observations, beliefs along the way
        for seg_id = 1 ... n = K_total / K_segment:
            (V_segment, π_segment, memo_segment) ← solve_bellman(b, K_segment, c)
            for k = 1 ... K_segment:
                a ← π_segment(b)
        ...
        g_m ← (pathwise via reverse-mode AD over K_total belief-update chain)
              + Σ_j (advantage A_j × per-segment score function at c)
        g += g_m
    g /= M
    c ← Adam_step(c, g, η)
...
The advantage `A_j` and pathwise term are derived in detail in `StochasticSegmentedDP.tex`,
§5.
...
|---|---|
| SSDP `n=4` `K_total=16`, `M=20`, `K_phi=128`, `T=500`, narrow prior | ~3 days |
| SSDP `n=8` `K_total=32`, `M=20`, `K_phi=128`, `T=500`, narrow prior | ~7 days |
| SSDP `n=16` `K_total=64`, `M=30`, `K_phi=64`, `T=500`, narrow prior | ~10 days |
...
Each baseline produces an MSE on the same paired-MC deployment; ratios against SSDP
...
iterates converge in expectation to a stationary point of `V_segmented(c)` at rate
`O(1/√T)`.  The estimator variance scales as `O(σ²/M)` where `σ²` depends on the spread of
segment-boundary belief values; advantage baselines reduce `σ²` substantially.  No smoothing
temperature anywhere; the only approximation is the receding-horizon truncation of the
segmented policy relative to the (intractable) full `K_total` Bellman, and even that
vanishes in the `n → 1` limit (where SSDP recovers exact joint-DP at the inner Bellman
horizon ceiling).

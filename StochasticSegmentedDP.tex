...
\title{Stochastic Segmented Dynamic Programming\\
...
ceiling.  We develop a scaling extension: \emph{stochastic segmented dynamic programming},
which combines (i) a segmented receding-horizon decomposition of the inner Bellman problem
into $n$ tractable segments of length $K_\mathrm{segment}$ and (ii) stochastic Monte-Carlo
sampling of segment-boundary trajectories at outer-gradient time, replacing the intractable
enumeration over reachable segment-boundary beliefs.  The resulting estimator is unbiased,
per-sample gradients are exact via the sharp-max envelope theorem, and the wall-clock cost
scales linearly in the sample size and the number of segments rather than in the reachable
...
\section{Segmented dynamic programming}
\label{sec:segmented}

Suppose the deployment horizon $K_\mathrm{total}$ is much larger than the exact-DP ceiling
$K_\mathrm{segment} \le K_\mathrm{max}$, with $K_\mathrm{total} = n \cdot K_\mathrm{segment}$
for some integer $n$.  Define a \emph{segmented policy} $\pi^\mathrm{segment}$ as follows:

\begin{enumerate}[nosep, leftmargin=2em]
\item At step $k = 0$, with belief $\bb_0 = p_0$, solve exact Bellman for $K_\mathrm{segment}$
      epochs starting at $\bb_0$.  Use the resulting policy $\pi^{(1)}$ for steps $1, \ldots, K_\mathrm{segment}$.
\item At step $k = K_\mathrm{segment}$, with belief $\bb_{K_\mathrm{segment}}$ resulting from
      executing $\pi^{(1)}$ on the realized observations, solve exact Bellman for $K_\mathrm{segment}$
      epochs starting at $\bb_{K_\mathrm{segment}}$.  Use $\pi^{(2)}$ for the next $K_\mathrm{segment}$ steps.
\item Repeat for $n$ segments total.
\end{enumerate}

Each segment is locally Bellman-optimal over its $K_\mathrm{segment}$-step subproblem; the
overall procedure is a receding-horizon approximation to the full $K_\mathrm{total}$
Bellman, suboptimal at the start (because $\pi^{(1)}$ plans only $K_\mathrm{segment}$ steps
ahead even though $K_\mathrm{total} - K_\mathrm{segment}$ steps remain) and tight at the end.

\subsection{The value of a segmented policy}

The expected reward under the segmented policy starting from $\bb_0 = p_0$ is
\begin{equation}
V^\mathrm{segment}(p_0;\, \bc)
\;=\;
\E_{\bb_{K_\mathrm{segment}} \sim P(\,\cdot\,\mid p_0, \pi^{(1)},\, \bc)}\!
\bigl[\,V^\mathrm{segment}(\bb_{K_\mathrm{segment}};\, \bc, \text{remaining } n - 1 \text{ segments})\,\bigr],
\label{eq:Vseg_recursive}
\end{equation}
unfolding to
\begin{equation}
V^\mathrm{segment}(p_0;\, \bc)
\;=\;
\sum_{\bb_{K_\mathrm{segment}}}\, P(\bb_{K_\mathrm{segment}} \mid p_0, \pi^{(1)},\, \bc)\,
V_0^{\mathrm{segment}_2}(\bb_{K_\mathrm{segment}};\, \bc),
\label{eq:Vseg_2seg}
\end{equation}
in the two-segment case ($n = 2$), where $V_0^{\mathrm{segment}_2}$ denotes the standard
$K_\mathrm{segment}$ Bellman value at the new starting belief $\bb_{K_\mathrm{segment}}$ over
the second segment.  The sum is over reachable segment-boundary beliefs.

\subsection{The curse of enumeration over segment-boundary beliefs}

To compute Equation~\eqref{eq:Vseg_2seg} exactly we would need
$V_0^{\mathrm{segment}_2}(\bb_{K_\mathrm{segment}};\, \bc)$ at every reachable
$\bb_{K_\mathrm{segment}}$.  In the count-tuple-keyed setting this is one fresh
$K_\mathrm{segment}$ Bellman solve per reachable count-tuple after segment~1.  At
$K_\mathrm{segment} = 4$ the reachable count-tuple set is $\sim 10^7$ in unfavourable
regimes, and each fresh solve costs $\sim 10$~seconds: enumeration cost is $10^8$ seconds
(thousands of CPU-years).

This is the same memo blow-up that capped exact $K_\mathrm{total} = K_\mathrm{segment} + 1$
in the first place, just relocated to the segment boundary.  Segmented DP, naively
implemented, does not break the exact-DP ceiling.

\section{Stochastic segmented DP}
...
$P(\bb_{K_\mathrm{segment}} \mid p_0, \pi^{(1)},\, \bc)$.

\subsection{The estimator}

Sample $M$ trajectories under the current segmented policy.  For each trajectory $m$ define
$\bb_{K_\mathrm{segment}}^{(m)}$ as the realized segment-1 terminal belief.  The estimator
\begin{equation}
\widehat{V}^\mathrm{segment}(p_0;\, \bc; \mathcal{M})
\;:=\;
\frac{1}{M} \sum_{m=1}^{M}
V_0^{\mathrm{segment}_2}\bigl(\bb_{K_\mathrm{segment}}^{(m)};\, \bc\bigr)
\label{eq:estimator}
\end{equation}
is an unbiased estimator of Equation~\eqref{eq:Vseg_2seg}: the trajectories
$\mathcal{M} = \{\bb_{K_\mathrm{segment}}^{(m)}\}_{m=1}^M$ are i.i.d.\ samples from the
correct policy-induced distribution, and the integrand is computed exactly per sample by
solving fresh $K_\mathrm{segment}$ Bellman from $\bb_{K_\mathrm{segment}}^{(m)}$.

The variance of the estimator scales as $\Var[V_0^{\mathrm{segment}_2}] / M$ where the
...
$K_\mathrm{segment}$-step subproblem starting at $\bb_{K_\mathrm{segment}}^{(m)}$:
\begin{enumerate}[nosep, leftmargin=2em]
\item Run forward $K_\mathrm{segment}$ Bellman from $\bb_{K_\mathrm{segment}}^{(m)}$.  Record
      the optimal argmax action at every reachable belief in the second segment's subtree.
\item Re-evaluate $V_0^{\mathrm{segment}_2}(\bb_{K_\mathrm{segment}}^{(m)};\, \bc)$ by
...
\begin{equation}
\frac{\partial V_0^{\mathrm{segment}_2}(\bb_{K_\mathrm{segment}}^{(m)};\, \bc)}{\partial \bc}
\;=\;
\left.\frac{\partial}{\partial \bc}\right|_{\pi^\star_{segment_2} \text{ frozen}}
V_0^{\mathrm{segment}_2}(\bb_{K_\mathrm{segment}}^{(m)};\, \bc).
\label{eq:envelope}
\end{equation}
The first segment's contribution to the gradient (through how $\pi^{(1)}$ shapes the sampling
distribution of $\bb_{K_\mathrm{segment}}$) follows the same envelope-theorem-frozen recursion
applied to segment 1, and contributes a score-function term in $\partial \log P / \partial \bc$
that can be carried via reverse-mode AD on the trajectory's likelihood, exactly as in the
single-segment joint-DP gradient.

The aggregate gradient
\begin{equation}
\widehat{\nabla}_{\bc} V^\mathrm{segment}(p_0;\, \bc; \mathcal{M})
\;=\;
\frac{1}{M} \sum_{m=1}^{M}
\frac{\partial V^{(m)}(\bc)}{\partial \bc}
\label{eq:agg_grad}
\end{equation}
is unbiased for $\nabla_{\bc} V^\mathrm{segment}(p_0;\, \bc)$ and has the standard SGD
variance scaling.
...
\begin{equation}
\bc^{(t+1)} = \mathrm{Adam}\bigl(\bc^{(t)},\; \widehat{\nabla}_{\bc} V^\mathrm{segment}(p_0;\, \bc^{(t)}; \mathcal{M}^{(t)}),\; \eta\bigr).
\end{equation}
...
expectation to a stationary point of $V^\mathrm{segment}(p_0;\, \bc)$.

\section{Gradient computation: a detailed derivation}
\label{sec:gradient}

This section derives the SSDP outer-hardware gradient explicitly.  We work the two-segment
case ($n = 2$) in full detail, identifying every $\bc$-dependence and unfolding each chain
rule.  We then generalize to $n > 2$ segments recursively.  No step is skipped; the goal is
that the formula in the algorithm box can be derived directly from first principles by
following this section.

\subsection{Setup for the two-segment case}
\label{sec:n2_setup}

For $n = 2$ with $K_\mathrm{total} = 2 K_\mathrm{segment}$, the segmented value is
\begin{equation}
V^\mathrm{segment}(p_0;\, \bc)
\;=\;
\E_{\bx \sim p_0}\,
\E_{(\by_1, \ldots, \by_{K_\mathrm{segment}}) \mid \bx, \pi^{[1]}(\bc), \bc}
\!\left[\,
V^{[2]}\bigl(\bb_{K_\mathrm{segment}}(\bx, \by_{1:K_\mathrm{segment}}; \bc);\, \bc\bigr)
\,\right],
\label{eq:Vseg_n2}
\end{equation}
where
\begin{itemize}[nosep,leftmargin=2em]
\item $\pi^{[1]}(\bc)$ is the segment-1 policy obtained by exact $K_\mathrm{segment}$ Bellman
      from $p_0$ at parameter $\bc$.  Each action in segment~1 is the local argmax
      $\bs_k = \pi^{[1]}(\bb_{k-1};\, \bc)$.
\item $\bb_{k} = T(\bb_{k-1}, \bs_k, \by_k;\, \bc)$ is the Bayesian belief update.
\item $V^{[2]}(\bb;\, \bc)$ is the standard $K_\mathrm{segment}$ Bellman value at starting
      belief $\bb$ with parameter $\bc$.  Because segment~2 is the last segment, its local
      terminal coincides with the global terminal $\Phi_0$.
\end{itemize}

The trajectory is the tuple $\tau := (\bx, \by_1, \ldots, \by_{K_\mathrm{segment}})$ with
joint density
\begin{equation}
P(\tau;\, \bc) \;=\; p_0(\bx) \cdot \prod_{k=1}^{K_\mathrm{segment}} p\bigl(\by_k \mid \bx, \bs_k(\tau_{<k}, \bc);\, \bc\bigr),
\label{eq:trajdensity}
\end{equation}
where $\bs_k(\tau_{<k}, \bc) = \pi^{[1]}(\bb_{k-1}(\tau_{<k}, \bc);\, \bc)$ is the
deterministic action under the segment-1 policy.

We want $\nabla_{\bc}\, V^\mathrm{segment}(p_0;\, \bc)$.

\subsection{Identifying every \texorpdfstring{$\bc$}{c}-dependence}
\label{sec:c_dependences}

Equation~\eqref{eq:Vseg_n2} has $\bc$ entering through five distinct paths.  We list
them explicitly because each contributes to the gradient differently:

\begin{enumerate}[nosep,leftmargin=2em]
\item[(P1)] \emph{Segment-1 policy actions} $\bs_k(\tau_{<k}, \bc)$ via the argmax of the local
      $K_\mathrm{segment}$ Bellman.
\item[(P2)] \emph{Within-segment-1 observation likelihood} $p(\by_k \mid \bx, \bs_k;\, \bc)$.
\item[(P3)] \emph{Within-segment-1 belief update} $T(\bb_{k-1}, \bs_k, \by_k;\, \bc)$, which
      shapes how $\bb_{K_\mathrm{segment}}$ depends on the trajectory.
\item[(P4)] \emph{Segment-2 Bellman value at fixed starting belief}
      $V^{[2]}(\cdot;\, \bc)$, which depends on $\bc$ through segment-2's likelihood and
      belief updates.
\item[(P5)] \emph{Starting belief input to segment-2}, $\bb_{K_\mathrm{segment}}(\bx, \by_{1:K_\mathrm{segment}}; \bc)$.
...
The segment-1 policy is $\bs_k(\bb_{k-1}; \bc) = \argmax_{\bs} Q^{[1]}_k(\bb_{k-1}, \bs;\, \bc)$.
...
$\bs^{(m)}_{1:K_\mathrm{segment}}$ on each sampled trajectory as
...
Let $F(\tau;\, \bc) := V^{[2]}(\bb_{K_\mathrm{segment}}(\tau, \bc);\, \bc)$.  Then
$V^\mathrm{segment}(p_0;\, \bc) = \E_{\tau \sim P(\cdot;\, \bc)}[F(\tau;\, \bc)]$.  By the
...
By chain rule on $F = V^{[2]}\bigl(\bb_{K_\mathrm{segment}}(\tau, \bc);\, \bc\bigr)$,
\begin{equation}
\nabla_{\bc} F(\tau;\, \bc)
\;=\;
\underbrace{\Bigl[\,\nabla_{\bb} V^{[2]}\!\bigl(\bb_{K_\mathrm{segment}};\, \bc\bigr)\,\Bigr]^\top \cdot \nabla_{\bc} \bb_{K_\mathrm{segment}}(\tau, \bc)}_{\text{(A): input-belief sensitivity}}
\;+\;
\underbrace{\biggl.\frac{\partial V^{[2]}\bigl(\bb;\, \bc\bigr)}{\partial \bc}\biggr|_{\bb \text{ held fixed at } \bb_{K_\mathrm{segment}}}}_{\text{(B): standard joint-DP gradient}}.
\label{eq:pathwise}
\end{equation}
...
$V^{[2]}$.  Specifically, holding the starting belief fixed at $\bb_{K_\mathrm{segment}}^{(m)}$
on the sampled trajectory, we run forward the $K_\mathrm{segment}$ Bellman recursion at
parameter $\bc$ to obtain the policy memo for segment~2.  Then we apply the sharp-max
envelope theorem: freeze the recorded action argmaxes throughout the segment-2 subtree, and
compute reverse-mode automatic differentiation on the resulting argmax-free expression for
$V^{[2]}(\bb_{K_\mathrm{segment}}^{(m)};\, \bc)$.  The result is exact for the segment-2
subproblem.

In the count-tuple-keyed setting, term (B) is computable in time $O(N_\mathrm{segment-2-memo} \cdot \mathrm{cost}(p, T))$, comparable to one $K_\mathrm{segment}$ Bellman solve.

\paragraph{Term (A): input-belief sensitivity through belief-update chain.}

This is two distinct factors:
\begin{itemize}[nosep, leftmargin=2em]
\item $\nabla_{\bb} V^{[2]}(\bb;\, \bc)$: the gradient of the segment-2 Bellman value with
      respect to the \emph{input belief} $\bb$, evaluated at $\bb = \bb_{K_\mathrm{segment}}^{(m)}$.
\item $\nabla_{\bc} \bb_{K_\mathrm{segment}}(\tau, \bc)$: the sensitivity of the segment-1
      terminal belief to $\bc$ along the realized segment-1 trajectory.
\end{itemize}

\textbf{Computing $\nabla_{\bb} V^{[2]}$.}  The segment-2 Bellman recursion at starting
...
\textbf{Computing $\nabla_{\bc} \bb_{K_\mathrm{segment}}$.}  The belief at the end of segment~1
is the recursion
\[
\bb_k = T(\bb_{k-1}, \bs_k, \by_k;\, \bc), \qquad k = 1, \ldots, K_\mathrm{segment},
\]
...
computable by one reverse-mode AD pass over the $K_\mathrm{segment}$-step chain.

\textbf{Combining (A1) and (A2).}  Term~(A) of Equation~\eqref{eq:pathwise} is
\[
\sum_{i=1}^{K_\Phi} \frac{\partial V^{[2]}}{\partial \bb_{K_\mathrm{segment}, i}}\bigl(\bb_{K_\mathrm{segment}}; \bc\bigr) \cdot \frac{\partial \bb_{K_\mathrm{segment}, i}}{\partial \bc}.
\]
...
treating the entire pipeline (segment-1 belief updates feeding segment-2 Bellman value) as a
...
g^{(m)}
\;=\;
\underbrace{\Bigl[\nabla_{\bb} V^{[2]}\bigr|_{\bb_{K_\mathrm{segment}}^{(m)}}\!\Bigr]^\top \cdot \nabla_{\bc} \bb_{K_\mathrm{segment}}^{(m)}}_{\text{Term A: belief-input × belief chain}}
\;+\;
\underbrace{\biggl.\frac{\partial V^{[2]}\bigl(\bb;\, \bc\bigr)}{\partial \bc}\biggr|_{\bb = \bb_{K_\mathrm{segment}}^{(m)}}}_{\text{Term B: joint-DP gradient at fixed input}}
\;+\;
\underbrace{V^{[2]}\bigl(\bb_{K_\mathrm{segment}}^{(m)};\, \bc\bigr) \cdot \sum_{k=1}^{K_\mathrm{segment}} \nabla_{\bc} \log p\bigl(\by_k^{(m)} \mid \bx^{(m)}, \bs_k^{(m)};\, \bc\bigr)}_{\text{Term C: score-function correction}}.
\label{eq:gm}
\end{equation}
...
\end{equation}
This estimator is unbiased for $\nabla_{\bc}\, V^\mathrm{segment}(p_0;\, \bc)$ by direct
...
segment-2 starting belief.  Term~A corrects for the fact that $\bb_{K_\mathrm{segment}}$ itself
...
proportional to the rate at which $\bb_{K_\mathrm{segment}}$ shifts under $\bc$, scaled by
...
\;=\;
\text{(A)} + \text{(B)} + \bigl[V^{[2]}(\bb_{K_\mathrm{segment}}^{(m)};\, \bc) - B(\bc)\bigr] \cdot \sum_k \nabla_{\bc} \log p(\by_k^{(m)} \mid \bx^{(m)}, \bs_k^{(m)}; \bc).
...
\begin{itemize}[nosep, leftmargin=2em]
\item \emph{Empirical mean:} $B(\bc) = \frac{1}{M} \sum_{m'=1}^M V^{[2]}(\bb_{K_\mathrm{segment}}^{(m')};\, \bc)$.
      ...
      remedy is leave-one-out: $B^{(m)}(\bc) = \frac{1}{M-1} \sum_{m' \ne m} V^{[2]}(\bb_{K_\mathrm{segment}}^{(m')};\, \bc)$.
\item \emph{Local Bellman value:} $B(\bc) = V^{[1]}_{\mathrm{local}}(p_0;\, \bc)$, the
      $K_\mathrm{segment}$ Bellman value of segment~1 at $p_0$ with local terminal $\Phi_0$.
      This is computable for free during the segment-1 Bellman solve and is the optimal
      baseline in the sense of minimum variance among $\bc$-only constants when the local
      terminal of segment~1 matches the global terminal of $V^{[2]}$.
\end{itemize}
...
applied at end of every segment including the final one).

\subsection{Generalization to \texorpdfstring{$n > 2$}{n > 2} segments}
\label{sec:nseg_general}

We now extend the derivation of \S\ref{sec:n2_setup}--\ref{sec:full_estimator} to general
$n$ segments.  The structure mirrors the two-segment case exactly; the additional content is
the per-segment decomposition of the score-function term and the corresponding advantage-form
variance reduction.

\subsubsection{Trajectory factorization across segments}
\label{sec:nseg_factorization}

For $n$ segments with $K_\mathrm{total} = n K_\mathrm{segment}$, denote the per-segment
sub-trajectories
\[
\tau_j := \bigl(\by_{(j-1)K_\mathrm{segment} + 1}, \ldots, \by_{j K_\mathrm{segment}}\bigr), \qquad j = 1, \ldots, n,
\]
and the full trajectory $\tau := (\bx, \tau_1, \ldots, \tau_n)$.  Let
$\bb^{\mathrm{bdy}}_j := \bb_{j K_\mathrm{segment}}$ denote the belief at the end of segment
...
\end{equation}
which by grouping shots into segments becomes
\begin{equation}
P(\tau;\, \bc)
\;=\;
p_0(\bx) \cdot \prod_{j=1}^{n} P_j\bigl(\tau_j \mid \bx, \bb^{\mathrm{bdy}}_{j-1};\, \bc\bigr),
\quad \text{where} \quad
P_j(\tau_j \mid \bx, \bb^{\mathrm{bdy}}_{j-1};\, \bc) = \prod_{k \in \text{segment } j} p(\by_k \mid \bx, \bs_k;\, \bc).
\label{eq:Pfactor_perseg}
\end{equation}
The per-segment density $P_j$ depends on $\bc$ through both the within-segment likelihoods and
the local Bellman policy that selects $\bs_k$ for $k \in$~segment~$j$.  As before, the policy
...
\label{eq:nseg_reinforce}
\end{equation}
Both terms unfold across all $K_\mathrm{total}$ steps; the per-segment decomposition arises
naturally because the underlying recursion has segment boundaries built into the policy.

\subsubsection{Pathwise gradient across all \texorpdfstring{$K_\mathrm{total}$}{K\_total} steps}
\label{sec:nseg_pathwise}
...
\label{eq:nseg_belief_chain}
\end{equation}
unfolding from $\partial \bb_0 / \partial \bc = 0$.  Equation~\eqref{eq:nseg_belief_chain}
is the same belief-chain recursion as Equation~\eqref{eq:belief_chain} but extended over all
$K_\mathrm{total}$ steps, not just the first $K_\mathrm{segment}$.  In implementation it is
one reverse-mode AD pass over the full trajectory's belief-update chain.  Note that
the segment boundaries do not appear specially in this term: the belief update is the same
operator at every step, regardless of which segment the step belongs to.

\paragraph{Comparison with the two-segment pathwise term.}  In the two-segment derivation
\eqref{eq:pathwise} the pathwise term split into an "input belief" piece~(A) and a
"fixed-input segment-2 Bellman gradient" piece~(B).  Here we have folded both pieces into
a single chain rule expression \eqref{eq:nseg_pathwise}.  The split is equivalent: the
"fixed-input segment-2 Bellman gradient" of the two-segment case corresponds to differentiating
the second segment's belief-update chain with the second segment's input frozen, while the
"input belief sensitivity" corresponds to differentiating the first segment's chain.  Both
flatten into the same total chain over $K_\mathrm{total}$ steps.

\subsubsection{Per-segment decomposition of the score-function term}
\label{sec:nseg_score}
...
\;=\;
\sum_{j=1}^{n} \underbrace{\nabla_{\bc} \log P_j\bigl(\tau_j \mid \bx, \bb^{\mathrm{bdy}}_{j-1};\, \bc\bigr)}_{\sum_{k \in \text{segment } j} \nabla_{\bc} \log p(\by_k \mid \bx, \bs_k;\, \bc)}.
\label{eq:nseg_score_decomp}
\end{equation}
The grouping into $n$ per-segment contributions is purely notational at this stage — the
per-segment sums are just additive.  But it becomes load-bearing in the next step, where
each per-segment term gets multiplied by the residual remaining-horizon value from that
segment onwards rather than by the global terminal.

\subsubsection{Equivalent per-segment score-function form (for variance reduction)}
\label{sec:nseg_perseg}

The "raw" REINFORCE gradient \eqref{eq:nseg_reinforce} multiplies the score function by
...
quantity for the score-function multiplier within each segment's term, without introducing
bias.  Specifically:
\begin{lemma}[Per-segment score-function multiplier substitution]
\label{lem:perseg_substitution}
For each $j = 1, \ldots, n$, let $W_j(\bc, \tau_{<j})$ be any function depending on $\bc$
and on the trajectory \emph{prior} to segment $j$ (i.e., on $\bx$ and on
...
This means we can replace the score-function multiplier inside segment $j$'s term with the
...
\label{eq:perseg_value_substitution}
\end{equation}
...
in segments $j+1, \ldots, n$ given the boundary belief), and that residual is independent of
segment $j$'s score function only when integrated against segment $j$'s score (which is what
Lemma~\ref{lem:perseg_substitution} certifies).

Combining \eqref{eq:nseg_reinforce}, \eqref{eq:nseg_score_decomp}, and
\eqref{eq:perseg_value_substitution} yields the \emph{per-segment} REINFORCE form:
\begin{equation}
\boxed{\;
\nabla_{\bc} V^\mathrm{segment}(p_0;\, \bc)
\;=\;
...
\subsubsection{Variance reduction with per-segment advantage baselines}
\label{sec:nseg_advantage}
...
introducing bias.  Define the per-segment advantage
\begin{equation}
...
\label{eq:advantage}
\end{equation}
This is the \emph{realized-minus-expected} value of segment $j$: it measures how much
unexpectedly more (or less) value segment $j$ delivered relative to what was anticipated
from $\bb^{\mathrm{bdy}}_{j-1}$ at the start of the segment.  By
Lemma~\ref{lem:perseg_substitution}, $\E[V_{\mathrm{global}}^{(j)}(\bb^{\mathrm{bdy}}_{j-1};\, \bc) \cdot \nabla_{\bc} \log P_j] = 0$, so substituting $V_{\mathrm{global}}^{(j+1)}$ with $A_j$ leaves the gradient unbiased:
\begin{equation}
\nabla_{\bc} V^\mathrm{segment}(p_0;\, \bc)
\;=\;
\E_\tau\!\Bigl[\,
...
\,\Bigr].
\label{eq:nseg_advantage}
\end{equation}
Equation~\eqref{eq:nseg_advantage} is the actor-critic / advantage form.  Variance is
typically much smaller than \eqref{eq:nseg_main} because $A_j$ has zero conditional mean
...
these in the general SSDP setting.  Three practical estimators:
\begin{itemize}[nosep, leftmargin=2em]
...
\item \emph{Local Bellman value}: $\widehat{V}_{\mathrm{global}}^{(j)}(\bb;\, \bc) \approx
  V^{[j]}_\mathrm{local}(\bb;\, \bc)$, the standard $K_\mathrm{segment}$ Bellman value with terminal
  $\Phi_0$ at end of segment $j$.  Free during the local Bellman solve.  Optimal as a baseline
  when local terminal coincides with global terminal.
\item \emph{Bootstrap from sampled trajectories}: simulate one further segment under the current
  policy starting from $\bb^{\mathrm{bdy}}_j$ and record its terminal value.  Equivalent to
  one-step temporal-difference estimation.
\end{itemize}
Any of these is unbiased as a baseline (Lemma~\ref{lem:perseg_substitution}) but they trade
off variance vs.\ implementation cost.

\subsubsection{Per-trajectory gradient estimator for general \texorpdfstring{$n$}{n}}
\label{sec:nseg_estimator}

Combining \eqref{eq:nseg_pathwise} and \eqref{eq:nseg_advantage}, the per-trajectory
...
\;+\;
\sum_{j=1}^{n} \underbrace{A_j^{(m)} \cdot \sum_{k \in \text{segment } j} \nabla_{\bc} \log p\bigl(\by_k^{(m)} \mid \bx^{(m)}, \bs_k^{(m)};\, \bc\bigr)}_{\text{per-segment advantage} \times \text{segment-} j \text{ score}}.
\label{eq:gm_nseg_full}
\end{equation}
Aggregating across $M$ samples:
\begin{equation}
\widehat{\nabla}_{\bc}\, V^\mathrm{segment}(p_0;\, \bc; \mathcal{M})
...
\end{equation}
This is unbiased for $\nabla_{\bc} V^\mathrm{segment}$ for any choice of advantage baselines
...
\item For each of $n$ segment boundaries: one $K_\mathrm{segment}$ Bellman solve to obtain
      $V^{[j]}_\mathrm{local}$ for the local-Bellman baseline (or alternative).  Cost:
      $O(n \cdot \mathrm{cost}(K_\mathrm{segment} \text{ Bellman}))$.
...
\end{itemize}
Total per sample: dominated by $n \cdot \mathrm{cost}(K_\mathrm{segment} \text{ Bellman})$ for
...
At $n = 2$, the per-segment sum in \eqref{eq:gm_nseg_full} contains two terms:
$j = 1$ (advantage of segment~1) and $j = 2$ (advantage of segment~2).
\begin{itemize}[nosep, leftmargin=2em]
\item $A_1 = V_{\mathrm{global}}^{(2)}(\bb^{\mathrm{bdy}}_1;\, \bc) - V_{\mathrm{global}}^{(1)}(p_0;\, \bc)$.
      ...
      $V^{[2]}(\bb_{K_\mathrm{segment}}^{(m)};\, \bc)$ that appeared as Term~C of
...
\item $A_2 = V_{\mathrm{global}}^{(3)}(\bb^{\mathrm{bdy}}_2;\, \bc) - V_{\mathrm{global}}^{(2)}(\bb^{\mathrm{bdy}}_1;\, \bc) = \Phi_0(\bb_{K_\mathrm{total}};\, \bc) - V^{[2]}(\bb^{\mathrm{bdy}}_1;\, \bc)$.
      The first piece is the realized terminal; the second is the predicted-from-segment-1-end value.
\end{itemize}
The pathwise gradient \eqref{eq:nseg_pathwise} over the $K_\mathrm{total} = 2 K_\mathrm{segment}$
...
Term~C.  The score-function term multiplied by $A_2$ contains the segment-2 score-function
contribution (which was implicitly handled inside Term~B in the two-segment derivation, since
Term~B used exact within-segment Bellman to integrate over segment-2's outcomes analytically).

In short: the $n = 2$ case of \eqref{eq:gm_nseg_full} is equivalent to
Equation~\eqref{eq:gm} after baseline shifting and after recognizing that the segment-2
score-function contribution is implicitly captured inside the segment-2 exact Bellman gradient.
The two derivations agree.

\paragraph{Why one might prefer $n = 2$'s explicit form.}  In the $n = 2$ derivation we kept
the second segment's exact-Bellman-with-summation-over-outcomes structure as Term~B; this
avoids one of the two stochastic score-function contributions and reduces variance.  For
general $n$, the analog is to use exact Bellman within the \emph{last} segment (which by
definition has terminal aligned with the global terminal) and stochastic segmented DP only
for segment boundaries $j = 1, \ldots, n-1$.  This hybrid keeps segment~$n$'s contribution
...
\subsection{Connection back to single-segment joint-DP}
\label{sec:single_seg_check}

For $n = 1$ (single segment with $K_\mathrm{total} = K_\mathrm{segment}$), there is no
segment-boundary sampling: the entire integral over $\tau$ is performed exactly inside the
Bellman recursion via the marginal observation probability sums.
Equation~\eqref{eq:gm_nseg_full} reduces to
\[
\nabla_{\bc} V^\mathrm{segment}(p_0;\, \bc)
\;=\;
\nabla_{\bc} V^{[1]}(p_0;\, \bc)
...
The score-function term vanishes because there are no segment boundaries and the $\E_\tau$
integration is performed exactly.  This recovers the exact-Bellman joint-DP gradient as
the $n = 1$ specialization of SSDP.  For $n \ge 2$ the additional score-function terms
appear precisely because the integration over segment-boundary trajectories is replaced by
Monte-Carlo sampling.

\section{The general algorithm}

\begin{algorithm}[H]
\caption{Stochastic Segmented DP}
\label{alg:ssdp}
\begin{algorithmic}[1]
\Require Initial $\bc^{(0)}$, total horizon $K_\mathrm{total}$, segment length $K_\mathrm{segment}$, sample size $M$, learning rate $\eta$, iterations $T$
\For{$t = 0, 1, \ldots, T - 1$}
    \State $g \gets 0$
    \For{$m = 1, \ldots, M$ (parallel)}
        \State $\bb \gets p_0$
        \State Initialize trajectory $\tau_m \gets ()$
        \For{$\mathrm{segment} = 1, \ldots, n = K_\mathrm{total} / K_\mathrm{segment}$}
            \State $(V_\mathrm{segment}, \pi_\mathrm{segment}, \mathrm{memo}_\mathrm{segment}) \gets \mathrm{Bellman}(\bb,\, K_\mathrm{segment},\, \bc^{(t)})$
            \For{$k = 1, \ldots, K_\mathrm{segment}$}
                \State $\bs \gets \pi_\mathrm{segment}(\bb)$
...
\textbf{Complexity per outer iteration}: $M \times n_\mathrm{segments} \times
\mathrm{cost}(K_\mathrm{segment} \text{ Bellman})$.  In typical regimes,
$\mathrm{cost}(K_\mathrm{segment} \text{ Bellman}) \approx 10$ seconds, so a single outer
...
by exact Bellman within its $K_\mathrm{segment}$ subproblem, with the sharp-max
...
$\widehat{V}^\mathrm{segment}(p_0;\, \bc; \mathcal{M})$ is unbiased for the true segmented
...
$V_0^{\mathrm{segment}_2}$ across reachable segment-boundary beliefs.  In well-conditioned
problems $\sigma^2$ is small and $M = 10$--$30$ suffices; in problems with bimodal or
multimodal segment-boundary belief distributions $M$ needs to grow accordingly.
...
$V^\mathrm{segment}(p_0;\, \bc)$ at the standard $1/\sqrt{T}$ rate.  Since the per-sample
gradient is exact rather than smoothed, the asymptotic noise floor is set by sampling
variance alone, not by gradient bias.

\paragraph{Approximation gap to full $K_\mathrm{total}$ Bellman.}  The segmented policy is a
strict approximation of the full $K_\mathrm{total}$ Bellman policy: each segment's lookahead
is truncated to $K_\mathrm{segment}$, missing the tail value of subsequent segments.  The gap
$V_0(p_0;\, \bc) - V^\mathrm{segment}(p_0;\, \bc) \ge 0$ is regime-dependent.  When the
segment-boundary posterior is unimodal and well-concentrated (e.g., $K_\mathrm{segment}$ is
large enough to "set up" a well-localized belief for subsequent segments), the gap is small.
When the segment-boundary posterior is multimodal, the gap can be large because segmented DP
...
(here, reachable segment-boundary beliefs) is replaced by a small stochastic mini-batch.  The
...
cutting-plane approximations of the value function.  Stochastic Segmented DP is the POMDP
analogue: scenarios are replaced by belief trajectories, cutting-plane approximations are
replaced by exact Bellman within each segment.  The receding-horizon decomposition is a
...
rather than exact Bellman.  Stochastic Segmented DP can be viewed as the upgrade of these
methods from "myopic" to "Bellman within each segment".
...
hardware near initialization (the "hardware-light fixed point").  Stochastic Segmented DP
sidesteps this by using exact Bellman within each segment, a non-NN inner solver that does
...
tree search over the belief space at policy-inference time.  Stochastic Segmented DP uses
...
\paragraph{Choosing $K_\mathrm{segment}$.}  Pick the largest $K_\mathrm{segment}$ for which
...
cost of suboptimal early-segment lookahead.
...
\paragraph{Diagnostics.}  Track the per-segment values $V_0^{\mathrm{segment}_j}$ and the
spread of $V^{(m)}$ across samples.  A high spread indicates that segment-boundary beliefs
are diverse, which is healthy for sampling but suggests $K_\mathrm{segment}$ may be too
small.  A monotonically decreasing segment-boundary belief variance across segments
indicates progressive disambiguation, which is the regime where segmented DP works well.

\section{Discussion}
\label{sec:discussion}

Stochastic Segmented DP fills a gap in the relaxation hierarchy between exact Bellman DP
...
solver within each segment and exactness of per-sample gradients via the envelope theorem,
...
$K_\mathrm{segment}$, (b) the deployment horizon is much larger, and (c) the outer hardware
...
representation makes the per-sample $K_\mathrm{segment}$ Bellman value $V_0^{\mathrm{segment}_j}(\bb)$
evaluatable at any belief $\bb$ in $O(|\Gamma| \cdot |\mathcal{X}|)$ time without
re-solving from scratch.  Combined with the stochastic-segmented outer SGD, this lifts the

# StochasticSegmentedDP project (CLAUDE briefing)

This file orients a Claude Code session that opens this folder cold.

## What this project is

Stochastic Segmented Dynamic Programming (SSDP): a scaling framework for joint
...
3. `StochasticSegmentedDP.tex` / `.pdf` — formal article.  §5 has the full step-by-step gradient derivation (n=2 detailed in §5.1-5.7, generalized n>2 detailed in §5.8 with Lemma 5.8.5 on score-function unbiasedness).
...
superconducting-qubit flux sensor).  SSDP extends that framework past the K=4-5 exact-DP
ceiling by stochastic outer SGD with exact inner Bellman per segment.
...
cannot disambiguate the multimodal posterior.  SSDP at K_total=16-32 should rescue the
...
measurements until τ becomes short enough to disambiguate.  SSDP-Bellman starts short to
...
- GPU is available but not used for SSDP (CPU-only Bellman recursion)
- BFIMGaussian repo at `/home/zlin/BFIMGaussian` for reference (different repo; SSDP no longer lives there as of commit `5ef7244`)
- Persistent auto-memory at `~/.claude/projects/-home-zlin-StochasticSegmentedDP/memory/`

## State at handoff

- Initial commit (`a45ae17` or successor) on `master`.
- New repo at `https://github.com/inversedesignai/StochasticSegmentedDP` (public).
- No experimental modules implemented yet — Phase 0 of the project charter is the next step.
- The 1-day Phase 1 calibration test (K=8 SSDP at φ=0.5 with dim(c)=7 to verify the framework numerically) is the natural first compute experiment after Phase 0 infrastructure work.

## When in doubt

- The `.tex` is the technical reference; the README is the project overview; the project charter is the operational plan.
- If a reader asks about a derivation step, point them to the corresponding subsection in `StochasticSegmentedDP.tex` §5.
